...
function validarFormulario(datos) {
    ...
    if (datos.nombreCompleto.length < 5) {
        errores.fullname = 'El nombre debe tener al menos 5 caracteres';
        esValido = false;
    }
    // Validación de caracteres permitidos usando regex
    else if (!REGEX_NOMBRE_VALIDO.test(datos.nombreCompleto)) {
        errores.fullname = 'El nombre solo puede contener letras y espacios';
        esValido = false;
    }
...
async function enviarFormulario(datos) {
    try {
        ...
        const data = await response.json(); // Procesa respuesta JSON
